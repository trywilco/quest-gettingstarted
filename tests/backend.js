...
const testItem = async () => {
    ...
    if (!getItem.data?.item.image) {
        console.log(`=!=!=!=!= ERROR: No placeholder image was found for item`);
        return false;
    }

    if (getItem.data?.item.image.includes("githubusercontent")) {
        console.log(
            `=!=!=!=!= ERROR: Please don't use images hosted on github for production use`
        );
        return false;
    }
